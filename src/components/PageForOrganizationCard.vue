...
const getOrgName = async () => {
  await fetch('http://89.23.116.186/api/education_programs')
      .then(res => res.json())
      .then(data => {
        counter.value = (data.filter((item) => item.id_organization == props.id));
      })
}
getOrgName()
    ...
    <div class="wrapper">
      <div class="container_for_logo_and_main_info">
        ...
        <div class="container_for_info">
          <h1>{{ currentOrganization?.fullTitle }}</h1>
...
{{ currentOrganization.description }}
          ...
          <div class="container_for_buttons">
            <a v-if="1" :href='1'>программы вуза</a>
            <a>новости вуза</a>
          </div>
        </div>
      </div>
      ...
      <div class="container_for_media_and_map">
        <div class="container_for_media">
          ...
          <p>Приемная комиссия: <span> {{ currentOrganization.phone }} </span></p>
