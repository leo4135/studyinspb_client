    ...
    <div class="container_for_cards" style="margin-top: 50px">
      <div class="card">
        ...
        <div class="text_block">
          <RouterLink class="org_hover" :to="{ name: 'organization', params: { id: prop.card.id } }"><h1>{{ prop.card.shortTitle }}</h1></RouterLink>
          <span class="areas" v-for="item in prop.card.oksoAreas">{{ item.toUpperCase() }}</span>
          <div class="container_for_attr_cards">
            <div class="container_for_text_and_logo">
              <div class="group_for_svg_and_count" v-if="prop.card.countPrograms.totalCount > 0">
              <svg width="39" height="38" viewBox="0 0 39 38" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <span> {{ prop.card.countPrograms.totalCount }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs"> образовательных <br> программ</span>
            </div>

            <div class="container_for_text_and_logo" v-if="prop.card.countPrograms.mag > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.mag }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs"> программ <br> магистратуры</span>
            </div>

            <div  class="container_for_text_and_logo" v-if="prop.card.countPrograms.bak > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.bak }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs"> программ <br> бакалавриата</span>
            </div>

            <div  class="container_for_text_and_logo" v-if="prop.card.countPrograms.asp > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.asp }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs">программ <br> аспирантуры</span>
            </div>

            <div  class="container_for_text_and_logo" v-if="prop.card.countPrograms.spec > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.spec }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs">программ <br> специалитета</span>
            </div>

            <div  class="container_for_text_and_logo" v-if="prop.card.countPrograms.spo > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.spo }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs">программ <br> СПО</span>
            </div>

            <div  class="container_for_text_and_logo" v-if="prop.card.countPrograms.dpo > 0">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ prop.card.countPrograms.dpo }} </span>
              </div>
