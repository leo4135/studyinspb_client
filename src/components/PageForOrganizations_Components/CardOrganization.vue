...
const getOrgName = async () => {
  await fetch('http://89.23.116.186/api/education_programs')
      .then(res => res.json())
      .then(data => {
        counter.value = (data.filter((item) => item.id_organization === props.card.id));
      })
}
getOrgName()
    ...
    <div class="container_for_cards" style="margin-top: 50px">
      <div class="card">
        ...
        <div class="text_block">
          <h1>{{ card.shortTitle.toUpperCase() }}</h1>
          ...
          <div class="container_for_attr_cards">
            ...
            <div class="container_for_text_and_logo">
              <div class="group_for_svg_and_count_other">
                ...
                <span> {{ counter.filter(item => item.qualification == 'Магистр').length }} </span>
              </div>
            ...
            <div class="container_for_text_and_logo">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ counter.filter(item => item.qualification == 'Бакалавр').length }} </span>
              </div>
            ...
            <div class="container_for_text_and_logo">
              <div class="group_for_svg_and_count_other">
                <div class="fake_svg"></div>
                <span> {{ counter.filter(item => item.qualification == 'Аспирант').length }} </span>
              </div>
              <span class="container_for_text_and_logo_education_programs">программ <br> аспирантуры</span>
            </div>
          </div>
          <RouterLink class="style_card" :to="{ name: 'organization', params: { id: card.id } }"><span>Подробнее</span></RouterLink>
        </div>
      </div>
    </div>
  </div>
